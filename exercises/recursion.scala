object Recursion {
    ...
    /** computes factorial of a number
      ...
      * 3! = 3 * 2 * 1
      */
    def fact(num:Int):Int = if (num==1 || num==0) 1 else num * fact(num-1)
    ...
    /** computes fibonacci value for a given number
      ...
      */
    def fib(num:Int):Int = if (num==0) 0 else if (num==1) 1 else fib(num-2)+fib(num-1) 

    /** returns true if 'list' contains 'needle'
      *
      * Tip: for maximum learning, implement list traversal via recursion
      */
    def contains[A](list: List[A], needle: A):Boolean = if (list.isEmpty) false else if (list.head==needle) true else contains[A](list.drop(1), needle)
     ...
     * with String.toList()
     */
    def length(s: String): Int = if (s.toList.isEmpty) 0 else length(s.substring(1))+1
